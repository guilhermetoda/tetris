...
ButtonScene::ButtonScene(const std::vector<std::string>& optionNames, const Color& textColor):
...
void ButtonScene::Update(uint32_t dt)
{
    
}

void ButtonScene::Draw(Screen& screen)
{
    for (auto& button: mButtons)
    {
        button.Draw(screen);
...
void ButtonScene::SetButtonActions(const std::vector<Button::ButtonAction>& buttonActions)
{
...
void ButtonScene::SetNextButtonHighlighted() {
    mHighlightedOption = (mHighlightedOption + 1) % mButtons.size();
    HighlightCurrentButton();
}
void ButtonScene::SetPreviousButtonHighlighted()
{
    --mHighlightedOption;
...
void ButtonScene::ExecuteCurrentButtonAction()
{
    mButtons[mHighlightedOption].ExecuteAction();
}

void ButtonScene::HighlightCurrentButton()
{
    for (auto& button: mButtons)
    {
