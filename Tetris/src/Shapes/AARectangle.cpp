...
void AARectangle::MoveTo(const Vec2D& position)
{
    ...
    SetBottomRightPont(Vec2D(position.GetX() + width -1, position.GetY() + height -1));
}
...
{
    return AARectangle(rect.GetTopLeftPoint() + insets, rect.GetWidth() - 2.0f*insets.GetX(), rect.GetHeight() - 2.0f*insets.GetY());
}
